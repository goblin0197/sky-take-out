...
//        checkOutOfRange(addressBook.getCityName()+addressBook.getDistrictName()+addressBook.getDetail());
    ...
    public PageResult pageQuery4User(int page, int pageSize, Integer status) {
        ...
        return new PageResult(orders.getTotal(), res);
    }
    ...
    public void userCancelById(Long id) {
        ...
        if (Orders.TO_BE_CONFIRMED.equals(orders.getStatus())) {
            // 调用微信支付退款接口
            //weChatPayUtil.refund( 商户订单号，商户退款号，退款金额，原订单金额 )
            // 支付状态改为 退款
    ...
    private String getOrderDishesStr(Orders orders) {
        ...
        orderDetailList.forEach(item -> {
            str.append(item.getName() + "*" + item.getNumber() + ";");
        });
        return str.toString();
    }
    ...
    public void reminder(Long id) {
        ...
        Map mp = new HashMap();
        mp.put("type", 2);//消息类型，2表示催单提醒
        mp.put("orderId", id);
        mp.put("content", "订单号：" + orders.getNumber());
